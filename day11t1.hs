...
solve :: String -> Int
solve raw = trace ("finalAnsArray = " ++ show finalAns) (top1 * top2)
    where
        [top1, top2] = take 2 $ List.reverse $ List.sort $ Array.elems finalAns
        (finalAns, finalMonkeys) = foldl oneRound (Array.listArray (0, numMonkeys - 1) (replicate numMonkeys 0), parsed) (replicate 20 ())
        ...
        evalExpr expr old = case expr of
                (Plus x y) -> (evalVal x + evalVal y) `div` 3
                (Mul x y) -> (evalVal x * evalVal y) `div` 3
            where
                evalVal Old = old
                evalVal (Num x) = x
