...
getMove :: RPS -> String -> RPS
getMove Rock "X" = Scissors
getMove Paper "X" = Rock
getMove Scissors "X" = Paper
getMove m "Y" = m
getMove Scissors "Z" = Rock
getMove Rock "Z" = Paper
getMove Paper "Z" = Scissors

score :: RPS -> RPS -> Int
score a b = (moveScore a) + (matchScore a b)

scoreOfLine s = helper $ split ' ' s
    where helper (x:y:[]) = score (getMove cx y) cx -- first move is opponent's move
            where cx = conv x
